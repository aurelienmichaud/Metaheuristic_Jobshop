...
public class ResourceOrder extends Encoding {
	...
	/*
	public static ArrayList<> permute(int[] arr) {
		if (arr.length = 1) {
			return arr;
		}
		...
		for (int x : ResourceOrder.permute(Arrays.copyOfRange(arr, 1, arr.length + 1))) {
			
		}
	}

	public static int reset = 0;
	public static ResourceOrder currentBruteForcedValue;

	private static Task[][] taskListByMachines;

	public static ResourceOrder bruteForceEnumeration(Instance instance) {
		if (reset == 1) {
			int[] nextSlot = new int[instance.numMachines];

			Arrays.fill(nextSlot, 0);
			currentBruteForcedValue = new ResourceOrder(instance);
			taskListByMachines = new Task[instance.numMachines][instance.numTasks];

			for (int job = 0; job < instance.numJobs; job++) {
				for (int task = 0; task < instance.numTasks; task++) {

					int machine = instance.machine(job, task);
					taskListByMachines[machine][nextSlot[machine]++] = new Task(job, task);
				}
			}
			System.out.println(taskListByMachines);
		}

		for (int machine = 0; machine < instance.numMachines; machine++) {
			for (int task = 0; task < instance.numTasks; task++) {
				currentBruteForcedValue.tasksByMachine[machine][task] = taskListByMachines[machine][task];
			}
		}

		return ResourceOrder.currentBruteForcedValue;
	}
	*/
